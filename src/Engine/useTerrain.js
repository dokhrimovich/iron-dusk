...
export const useTerrain = ({ ctx, skeleton, scale, map, enteties }) => {
    ...
    const d90x85 = useMemo(() => [cellWidth/2 + cellWidth/4, cellHeight + cellHeight/2, cellWidth + cellWidth/2, cellHeight*2 + cellHeight/2], [cellWidth, cellHeight]);
    const d120x136 = useMemo(() => [cellWidth + cellWidth/2, cellHeight*3, cellWidth*2, cellHeight*4], [cellWidth, cellHeight]);
    ...
    const drawSprite90x85 = useCallback((x, y, img) => {
        const [dx, dy, cw, ch] = d90x85;
        const [cx, cy] = [(x - dx) , (y - dy)];

        safeDrawImage(ctx, img, 0, 0, 90, 85, cx, cy, cw, ch);
    }, [ctx, d90x85]);
    ...
    const drawGrass = useCallback(() => {
        ...
        skeleton.forEach((row, ri) => {
            row.slice().reverse().forEach((col, cri) => {
                ...
                if (map[ri][ci] === 2) {
                    drawSprite90x85(x, y, images.grass_back01);
                    someone?.type === 1 && drawSprite160x136(x, y, images.warrior01);
                    drawSprite90x85(x, y, images.grass_front01);
                } else if (someone?.type === 1) {
                    drawSprite160x136(x, y, images.warrior01);
                }
            });
        });
